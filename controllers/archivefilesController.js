...
exports.getFilesByName = async (req, res) => {
  ...
  try {
    ...
    const data = await db.ArchiveFiles.findByName(
      projectsNames,
      role,
      fileName,
      viewMode
...
exports.getFilesByFilters = async (req, res) => {
  ...
  try {
    const db = await getDb();
    const data = await db.ArchiveFiles.findAllByFilters(
      startDate,
      endDate,
      status,
      worldContents,
...
exports.UpdateFilesStatus = async (req, res) => {
  const { files, status } = req.body;
  try {
    const db = await getDb();
    await db.ArchiveFiles.updateStatus(files, status);
    return res.send({
      status: "success",
    });
  } catch (error) {
